...
namespace ALL_IN
{
    public class LocationService
    {
        ...
        public void UpdateLocation(Location location)
        {
            ...
            catch (Exception e) { PrintMessage(e.ToString()); }
        }
        public void UpdateLocation(string Lon,string Lat)
        {
            try
            {
                if (Devices.Count == 0)
        ...
        public void AutoUpdateLocation(Location location, int i)
        {
            try
            {
                if (Devices.Count == 0)
                ...
                Devices.ForEach(itm =>
                {
                    ...
                    //device.Dispose();
                    //client.Dispose();
                    PrintMessage($"第" + i + "个坐标点已到达。");
                });
            }
            catch (Exception e) { PrintMessage(e.ToString()); }
        }
        public void AutoUpdateLocation(string Lon, string Lat, int i)
        {
            try
            {
                if (Devices.Count == 0)
